...
class TeamsDownloaderUtil():
    ...
    async def init_http(self, in_cookies: Dict, in_headers: Dict):
        self.http_client = aiohttp.ClientSession(
            cookies=in_cookies, headers=in_headers)
    ...
    async def download_file(self, url, folder, file_override: str = None):
        ...
        async with self.http_client.get(url, stream=True) as r:
            async with aiofiles.open(normalize_str(folder) + '/' + local_filename, 'wb') as f:
                await asyncio.to_thread(shutil.copyfileobj, r.raw, f)
